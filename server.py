...
def normalize_store(value):
    """Accept a store handle or its canonical myshopify.com URL."""
    ...
    if not re.fullmatch(r"[a-z0-9][a-z0-9-]*", value):
        raise RuntimeError(
            "SHOPIFY_STORE must be a store handle or myshopify.com domain."
        )

    return value


SHOPIFY_STORE = normalize_store(
    os.environ.get("SHOPIFY_STORE", "gz1iyb-qp")
)
...
TOKEN_REFRESH_BUFFER = int(
    os.environ.get("TOKEN_REFRESH_BUFFER", "1800")
)
...
class TokenManager:
    def __init__(
        ...
        self._use_client_credentials = bool(
    ...
    async def get_token(self):
        ...
        async with self._lock:
            ...
            if self._use_client_credentials:
                ...
                raise RuntimeError(
                    "No valid token. Set SHOPIFY_ACCESS_TOKEN."
                )
    ...
    async def _refresh_token(self):
        url = (
            f"https://{self._store}.myshopify.com"
...
async def _request(
    ...
    async with httpx.AsyncClient() as client:
        ...
        if (
            ...
            and not _retried
            and token_manager._use_client_credentials
        ):
...
async def shopify_get_product(
    ...
    try:
        data = await _request(
            "GET",
            f"products/{params.product_id}.json",
        )
...
async def shopify_count_products(
    params: ProductCountInput,
) -> str:
    """Count products."""
    try:
        p = {}
...
async def shopify_get_customer(
    ...
    try:
        data = await _request(
            "GET",
            f"customers/{params.customer_id}.json",
        )
...
async def shopify_list_themes(
    params: ListThemesInput,
) -> str:
    """List installed themes. The 'main' role is the active theme."""
    try:
        p = {}
...
async def shopify_list_theme_assets(
...
async def shopify_get_theme_asset(
...
async def shopify_update_theme_asset(
    params: UpdateAssetInput,
) -> str:
    """Create or update a theme asset. Overwrites existing content."""
    try:
...
async def shopify_delete_theme_asset(
    ...
    try:
        ...
        return (
            f"Asset '{params.asset_key}' deleted "
            f"from theme {params.theme_id}."
        )
...
class APIKeyMiddleware:
    ...
    def __init__(self, app, api_key):
        if len(api_key) < 32:
            raise RuntimeError(
                "Set MCP_API_KEY to a random key "
...
if __name__ == "__main__":
    ...
    if MCP_TRANSPORT != "streamable-http":
        raise RuntimeError(
            ...
            "for this protected remote server."
        )
